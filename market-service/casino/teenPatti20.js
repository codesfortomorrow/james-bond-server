...
class TeenPatti20 {
  ...
  constructor() {
    this.casino = 'Teenpatti-20-20';
    this.currentMarket = null;
    this.markets = [];
  }

  async casinoHandler(producer) {
    const response = await getData(process.env.TEEN_PATTI_20_MARKET_ENDPOINT);
    ...
    if (response.success === true && typeof response.data === 'object') {
      producer.send({
        topic: kafkaTopics.CASINO_TEEN_PATTI_20_CATALOGUE.topic,
        messages: [
  ...
  async getCasinoResults(producer) {
    const response = await getData(process.env.TEEN_PATTI_20_RESULT_ENDPOINT);
    if (response instanceof Error) {
      return;
    }
      ...
      this.markets.forEach((mid, index) => {
        for (let i = 0; i < response.data.length; i++) {
          if (response.data[i].mid === mid) {
            ...
            console.debug(`Message emit ${kafkaTopics.CASINO_TEEN_PATTI_20_RESULT.topic}: ${JSON.stringify({ marketId: Number(response.data[i].mid), result: Number(response.data[i].result), casino: this.casino })}`);
            producer.send({
              topic: kafkaTopics.CASINO_TEEN_PATTI_20_RESULT.topic,
              messages: [
