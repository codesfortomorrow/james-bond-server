...
class Tennis {
  ...
  async fixtureHandler(producer) {
    ...
    const competitions = await getData(`${process.env.TENNIS_COMPETITIONS_ENDPOINT}?id=2`);
    ...
    await Promise.all(competitions.map(async (c) => {
      const events = await getData(`${process.env.TENNIS_EVENTS_ENDPOINT}?sid=${c.competition.id}&sportid=2`);
      if (!events instanceof Error || events.length>0) {
        events.forEach((e) => {
          const currentDate = new Date();
            ...
            if (Math.round(daysDifference) <= 3  && daysDifference >= -1 ) {
    ...
    for (let key in eventHolder) {
      const marketids = await getData(`${process.env.TENNIS_EVENTS_MARKET}?EventID=${key}&sportid=2`);
      if (marketids instanceof Error || !(marketids instanceof Array)){
        continue
      }
      ...
      const odds = await getData(`${process.env.TENNIS_MARKET_ENDPOINT}?market_id=${eventHolder[key]['market_id']}&sportid=2`);
   

   
      if (!Array.isArray(odds) || odds instanceof Error || !(odds instanceof Array || odds.length === 0) || !odds[0]) {
        // console.error('No data received for odds');
        continue;
    } 
  ...
  async competitionHandler(producer) {
    const response = await getData(`${process.env.TENNIS_COMPETITIONS_ENDPOINT}?id=2`);
    if (response instanceof Error || !(response instanceof Array)) {
      producer.send({
        topic: kafkaTopics.TENNIS_COMPETITION_LIST.topic,
        messages: [
          { value: JSON.stringify([]) }
        ]
      });
  ...
  async eventHandler(producer, competitionId) {
    const response = await getData(`${process.env.TENNIS_EVENTS_ENDPOINT}?sid=${competitionId}&sportid=2`);
    if (response instanceof Error) {
      producer.send({
        topic: kafkaTopics.TENNIS_EVENT_LIST.topic,
        messages: [
    ...
    if (newEvents.length) {
      this.eventEmitter.emit('data', competitionId, newEvents.map(id => ({
        eventId: id,
        // marketId: eventById[id].marketId,
        // runners: eventById[id].runners.reduce((obj, cur) => ({ ...obj, [cur.selectionId]: cur.runnerName }), {}),
        competitionId,
  ...
  async marketHandler(producer, competitionId, eventId, event) {
    ...
    const response = await getData(`${process.env.TENNIS_EVENTS_MARKET}?EventID=${eventId}&sportid=2`);
    
    if (response instanceof Error || !(response instanceof Array) || !Array.isArray(response) || response.length === 0){
      producer.send({
        topic: kafkaTopics.TENNIS_MARKET_CATALOGUE.topic,
        messages: [
          {
    ...
    const filteredMarkets = response.filter(market =>
     market.marketName === 'Set 1 Winner' ||
      market.marketName === 'Set 2 Winner' ||
...
if (Object.hasOwnProperty.call(marketIds, marketId)) {
  const response = await getData(`${process.env.TENNIS_MARKET_ENDPOINT}?market_id=${marketId}`);
      ...
      markets.map(market => {
        if (!this.marketResultIntervals[market]) {
          const interval = setInterval(async () => {
            const result = await getData(`${process.env.TENNIS_RESULT_ENDPOINT}?market_id=${market}&sportid=2`);
            ...
            if (result instanceof Array) {
              result.map(m => {
                if (m.status === 'CLOSED') {
                  ...
                  m.runners.map(r => {
                    // console.debug(`Message emit ${kafkaTopics.TENNIS_MARKET_RESULT.topic}: ${JSON.stringify({ gameId: 2, eventId: Number(eventId), marketId: Number(m.marketId), selectionId: Number(r.selectionId), market: 'MATCH_ODDS', gameType: 'match', status: r.status })}`);
                    if (m.marketName === 'Match Odds') {
                    producer.send({
                      topic: kafkaTopics.TENNIS_MARKET_RESULT.topic,
                      messages: [
                        { value: JSON.stringify({ gameId: 2, eventId: Number(eventId), marketId: String(m.marketId), selectionId: String(r.selectionId), market: 'MATCH_ODDS', gameType: 'match', status: r.status }) }
                      ]
                    });
                  }
                  else{
                    producer.send({
                      topic: kafkaTopics.TENNIS_MARKET_RESULT.topic,
                      messages: [
                        { value: JSON.stringify({ gameId: 2, eventId: Number(eventId), marketId: String(m.marketId), selectionId: String(r.selectionId), market: m.marketName, gameType: m.marketName, status: r.status }) }
                      ]
                    });
                  }
                  });
